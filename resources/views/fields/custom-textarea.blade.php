<style>
    ...
    .cta-wrapper .cta-input:focus {
        border-color: #3b82f6;
        box-shadow: 0 0 0 1px #3b82f6;
    }

    .cta-wrapper .cta-input.cta-error {
...
}" class="cta-wrapper" x-cloak>
    <div class="cta-input-container">
        <textarea {!! $element->getAttributes()->merge(['class' => 'cta-input form-textarea']) !!}
                  name="{{ $element->getName() }}"
                  x-model="value"
                  @blur="validate()"
                  :class="{'cta-error': error !== null}"
        >{{ $element->getValue() }}</textarea>
        <div class="cta-icon-wrapper">
            <template x-if="error !== null">
                <div class="cta-icon-error">
