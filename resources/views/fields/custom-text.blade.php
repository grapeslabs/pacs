<style>
    ...
    .ctf-spinner {
        ...
        border-radius: 50%;
        animation: ctf-spin 1s linear infinite;
    }
...
<div x-data="{
    ...
    timeout: null,
    init() {
        if (!window.textFieldsInitialized) {
            window.textFieldsInitialized = true;
            document.addEventListener('submit', function(event) {
                const errorField = document.querySelector('.ctf-error');
                if (errorField) {
    ...
    validate() {
        ...
        for (let rule of this.rules) {
            ...
            if (rule.type === 'pattern' && !(new RegExp(rule.value)).test(this.value)) { this.error = rule.message; return; }
            if (rule.type === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.value)) { this.error = rule.message; return; }
            if (rule.type === 'url' && !/^(https?:\/\/)/.test(this.value)) { this.error = rule.message; return; }
            if (rule.type === 'ipv4' && !/^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/.test(this.value)) { this.error = rule.message; return; }
            if (rule.type === 'ipv6' && !/^[a-fA-F0-9:]+$/.test(this.value)) { this.error = rule.message; return; }
            if (rule.type === 'alpha' && !/^[А-Яа-яA-Za-zЁё\s]+$/.test(this.value)) { this.error = rule.message; return; }
            if (rule.type === 'alphaNum' && !/^[А-Яа-яA-Za-zЁё0-9\s]+$/.test(this.value)) { this.error = rule.message; return; }
            if (rule.type === 'phone' && !/^[0-9\+\-\(\)\s]+$/.test(this.value)) { this.error = rule.message; return; }
    ...
    async checkAjax(rule, allAjaxRules) {
        try {
            const response = await fetch('/field-validation', {
                method: 'POST',
                headers: {
...
}" class="ctf-wrapper" x-cloak>
    <div class="ctf-input-container">
        <input {!! $element->getAttributes()->merge(['class' => 'ctf-input form-input']) !!}
               type="text"
               name="{{ $element->getName() }}"
               x-model="value"
               @blur="validate()"
               :class="{'ctf-error': error !== null}"
        />
        <div class="ctf-icon-wrapper">
            <template x-if="loading">
                <div class="ctf-spinner"></div>
            </template>
            <template x-if="error !== null && !loading">
                <div class="ctf-icon-error">
