<style>
    ...
    .cdf-input::-webkit-calendar-picker-indicator {
        position: absolute;
        right: 0.75rem;
        cursor: pointer;
    }
...
<div x-data="{
    ...
    timeout: null,
    init() {
        if (!window.customDateFieldsInitialized) {
            window.customDateFieldsInitialized = true;
            document.addEventListener('submit', function(event) {
                const errorField = document.querySelector('.cdf-error');
                if (errorField) {
    ...
    validate() {
        ...
        let myDate = Date.parse(this.value);
        ...
        for (let rule of this.rules) {
            ...
            if (rule.type === 'before') {
                if (myDate >= Date.parse(rule.value)) { this.error = rule.message; return; }
            }
            if (rule.type === 'after') {
                if (myDate <= Date.parse(rule.value)) { this.error = rule.message; return; }
            }

            if (['beforeField', 'afterField', 'sameField'].includes(rule.type)) {
                let targetInput = document.querySelector(`input[name='${rule.field}']`);
                if (targetInput && targetInput.value) {
                    let targetDate = Date.parse(targetInput.value);
                    if (!isNaN(targetDate)) {
                        if (rule.type === 'beforeField' && myDate >= targetDate) { this.error = rule.message; return; }
                        if (rule.type === 'afterField' && myDate <= targetDate) { this.error = rule.message; return; }
                        if (rule.type === 'sameField' && myDate !== targetDate) { this.error = rule.message; return; }
                    }
                }
            }
        }
    ...
    async checkAjax(rule, allAjaxRules) {
     ...
     class="cdf-wrapper" x-cloak>
    <div class="cdf-input-container">
        <input {!! $element->getAttributes()->merge(['class' => 'cdf-input form-input', 'type' => 'date']) !!}
               name="{{ $element->getName() }}"
               x-model="value"
               @blur="validate()"
               :class="{'cdf-error': error !== null}"
        />
        <div class="cdf-icon-wrapper">
            <template x-if="loading">
                <div class="cdf-spinner"></div>
            </template>
            <template x-if="error !== null && !loading">
                <div class="cdf-icon-error">
